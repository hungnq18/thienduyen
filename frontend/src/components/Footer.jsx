...
const Footer = () => {
  return (
    <footer className="w-full h-[245px] bg-[#610912] relative">
      {/* Logo Section */}
      <div className="absolute left-[36px] top-[54px] w-[182px] h-[182px]">
        <img src={logo} alt="logo" className="w-[144px] h-[144px]" />
      </div>

      {/* Chính sách Section */}
      <div className="absolute left-[345px] top-[61px]">
        <h4 className="text-[#FFFFFF] font-bold text-2xl mb-4" style={{ fontFamily: 'Arimo, sans-serif' }}>Chính sách</h4>
        <div className="text-[#FFFFFF] text-lg space-y-2" style={{ fontFamily: 'Arimo, sans-serif', fontWeight: 400 }}>
          <div>Điều khoản sử dụng</div>
          <div>Chính sách bảo mật</div>
          <div>Tính trợ năng</div>
        </div>
      </div>

      {/* Liên hệ Section */}
      <div className="absolute left-[553px] top-[61px]">
        <h4 className="text-[#FFFFFF] font-bold text-2xl mb-4" style={{ fontFamily: 'Arimo, sans-serif' }}>Liên hệ</h4>
        <div className="text-[#FFFFFF] text-lg space-y-2" style={{ fontFamily: 'Arimo, sans-serif', fontWeight: 400 }}>
          <div>Trường Đại học FPT</div>
          <div>Điện thoại : 091 338 3603</div>
          <div>Email: thienduyenwedding@gmail.com</div>
        </div>
      </div>

      {/* Newsletter Section */}
      <div className="absolute left-[853px] top-[45px] w-[558px] h-[66px] flex items-center gap-6">
        {/* Email Input */}
        <div className="w-[419px] h-[66px] border border-white rounded-xl flex items-center px-6">
          <input
            type="email"
            placeholder="Enter your email"
            className="w-full bg-transparent text-white placeholder-white text-base outline-none"
        ...
        <a 
          href="#"
          className="w-[112px] h-[54px] bg-[#610912] rounded-[30px] text-white font-bold text-base hover:bg-white hover:text-[#610912] transition-colors flex items-center justify-center"
          style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 700, color: '#FFFFFF' }}
          onMouseEnter={(e) => e.target.style.color = '#610912'}
